...
def train(net, loss, train_dataloader, valid_dataloader, device, batch_size,
          num_epoch, lr, lr_min, optim='adam', init=True, scheduler_type='Cosine'):
    ...
    if optim == 'sgd':
        ...
        optimizer = torch.optim.AdamW((param for param in net.parameters() if param.requires_grad),
                                      lr=lr, weight_decay=0)
...
if __name__ == "__main__":
    ...
    def get_dataloader(batch_size):
        data_transform = {
            ...
            "val": transforms.Compose([transforms.Resize(256),
                                       ...
                                       transforms.ToTensor(),
                                       transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])}
        train_dataset = torchvision.datasets.CIFAR10('../datasets/cifar/', train=True,
                                                     transform=data_transform["train"], download=True)
        test_dataset = torchvision.datasets.CIFAR10('../datasets/cifar/', train=False,
    ...
    def show_acces(train_losses, train_acces, valid_acces, num_epoch):  # 对准确率和loss画图显得直观
        plt.plot(1 + np.arange(len(train_losses)), train_losses, linewidth=1.5, linestyle='dashed',
                 label='train_losses')
    ...
    train_losses, train_acces, eval_acces = train(net, loss, train_dataloader, test_dataloader, device, batch_size=64,
                                                  num_epoch=20, lr=0.1, lr_min=1e-4, optim='sgd', init=False)
